...
/**
 * <code>TimeZone</code> represents a time zone offset, and also figures out
 * daylight savings.
 * 
 * <p>
 * Typically, you get a <code>TimeZone</code> using <code>getDefault</code>
 * which creates a <code>TimeZone</code> based on the time zone where the
 * program is running. For example, for a program running in Japan,
 * <code>getDefault</code> creates a <code>TimeZone</code> object based on
 ...
 * <code>TimeZone</code> object with: <blockquote>
 * 
 * <pre>
 * TimeZone tz = TimeZone.getTimeZone(&quot;America/Los_Angeles&quot;);
 * </pre>
 * 
 * </blockquote> You can use the <code>getAvailableIDs</code> method to iterate
 * through all the supported time zone IDs. You can then choose a supported ID
 * to get a <code>TimeZone</code>. If the time zone you want is not represented
 * by one of the supported IDs, then a custom time zone ID can be specified to
 ...
 * </blockquote>
 * 
 * <i>Hours</i> must be between 0 to 23 and <i>Minutes</i> must be between 00 to
 * 59. For example, "GMT+10" and "GMT+0010" mean ten hours and ten minutes ahead
 * of GMT, respectively.
 * <p>
 * The format is locale independent and digits must be taken from the Basic
 * Latin block of the Unicode standard. No daylight saving time transition
 * schedule can be specified with a custom time zone ID. If the specified string
 * doesn't match the syntax, <code>"GMT"</code> is used.
 * <p>
 * When creating a <code>TimeZone</code>, the specified custom time zone ID is
 * normalized in the following syntax: <blockquote>
 * 
 * <pre>
 ...
 * </pre>
 * 
 * </blockquote> For example, TimeZone.getTimeZone("GMT-8").getID() returns
 * "GMT-08:00".
 * 
...
abstract public class TimeZone implements Serializable, Cloneable {
	...
	/**
	 * A style specifier for <code>getDisplayName()</code> indicating a short
	 * name, such as "PST."
	 * 
	 * @see #LONG
	 * @since 1.2
	...
	/**
	 * A style specifier for <code>getDisplayName()</code> indicating a long
	 ...
	 * <p>
	 * This method returns a historically correct offset if an underlying
	 * <code>TimeZone</code> implementation subclass supports historical
	...
	/**
	 * Gets the raw GMT offset and the amount of daylight saving of this time
	 * zone at the given time.
	 * 
	 * @param date
	 *            the milliseconds (since January 1, 1970, 00:00:00.000 GMT) at
	 *            which the time zone offset and daylight saving amount are
	 *            found
	 * @param offset
	 *            an array of int where the raw GMT offset (offset[0]) and
	 *            daylight saving amount (offset[1]) are stored, or null if
	 *            those values are not needed. The method assumes that the
	 *            length of the given array is two or larger.
	 * @return the total amount of the raw GMT offset and daylight saving at the
	 *         specified date.
	 * 
	 * @see Calendar#ZONE_OFFSET
	 * @see Calendar#DST_OFFSET
	 ...
	 * to get local time.
	 * <p>
	 * If an underlying <code>TimeZone</code> implementation subclass supports
	 * historical GMT offset changes, the specified GMT offset is set as the
	 * latest GMT offset and the difference from the known latest GMT offset
	 * value is used to adjust all historical GMT offset values.
	 * 
	 * @param offsetMillis
	 ...
	 * time in this time zone. Because this value is not affected by daylight
	 * saving time, it is called <I>raw offset</I>.
	 * <p>
	 * If an underlying <code>TimeZone</code> implementation subclass supports
	 * historical GMT offset changes, the method returns the raw offset value of
	 * the current date. In Honolulu, for example, its raw offset changed from
	 * GMT-10:30 to GMT-10:00 in 1947, and this method always returns -36000000
	 * milliseconds (i.e., -10 hours).
	 * 
	 ...
	 * the default locale. This method returns the long name, not including
	 * daylight savings. If the display name is not available for the locale,
	 * then this method returns a string in the <a
	 * href="#NormalizedCustomID">normalized custom ID format</a>.
	 * 
	 ...
	 * @param style
	 *            either <code>LONG</code> or <code>SHORT</code>
	 * @return the human-readable name of this time zone in the default locale.
	 * @since 1.2
	 */
	 ...
	 * locale, then this method returns a string in the <a
	 * href="#NormalizedCustomID">normalized custom ID format</a>.
	 * 
	 * @param daylight
	 *            if true, return the daylight savings name.
	 ...
	 * <p>
	 * If an underlying <code>TimeZone</code> implementation subclass supports
	 * historical Daylight Saving Time schedule changes, the method refers to
	 * the latest Daylight Saving Time schedule information.
	 * 
	 ...
	 *         otherwise.
	 */
	abstract public boolean inDaylightTime(DateShakyou date);
	...
	private static TimeZone getTimeZone(String ID, boolean fallback) {
		...
		// java.util.TimeZone tz = ZoneInfo.getTimeZone(ID);
		if (tz == null) {
			tz = parseCustomTimeZone(ID);
			// (com.shakyou.TimeZone)tz = parseCustomTimeZone(ID);
			if (tz == null && fallback) {
	...
	private static synchronized TimeZone setDefaultZone() {
		...
		String zoneID = AccessController.doPrivileged(new GetPropertyAction(
				"user.timezone"));

		// if the time zone ID is not set (yet), perform the
		// platform to Java time zone ID mapping.
		if (zoneID == null || zoneID.equals("")) {
			String country = AccessController
					.doPrivileged(new GetPropertyAction("user.country"));
			String javaHome = AccessController
	...
	private static boolean hasPermission() {
		...
		if (sm != null) {
			try {
				sm.checkPermission(new PropertyPermission("user.timezone",
						"write"));
	 ...
	 * <code>getDefault</code> method. If <code>zone</code> is null, reset the
	...
	private static final InheritableThreadLocal<TimeZone> defaultZoneTL = new InheritableThreadLocal<TimeZone>();
