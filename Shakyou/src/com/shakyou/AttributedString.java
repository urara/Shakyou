...
public class AttributedString {
	 ...
	 * @exception NullPointerException
	 *                if <code>text</code> or <code>attributes</code> is null.
	 ...
	 *            range.
	 * @exception NullPointerException
	 *                if <code>attributes</code> is null.
	 * @exception IllegalArgumentException
	 *                if beginIndex is less then 0, endIndex is greater than the
	 *                length of the string, or beginIndex and endIndex together
	...
	public void addAttributes(Map<? extends Attribute, ?> attributes,
		...
		while (iterator.hasNext()) {
			Map.Entry entry = (Map.Entry) iterator.next();
			addAttributeRunData((Attribute) entry.getKey(), entry.getValue(),
	 ...
	 * <ul>
	 * <li>If copyAttrs is true, the attributes from the existing run will be
	 * placed in both of the newly created runs.
	 * <li>If copyAttrs is false, the attributes from the existing run will NOT
	 * be copied to the run to the right (>= offset) of the break, but will
	 ...
	 * the entire contents of this string.
	 * 
	...
	/**
	 * Creates an AttributedCharacterIterator instance that provides access to
	 * selected contents of this string. Information about attributes not listed
	 * in attributes that the implementor may have need not be made accessible
	 * through the iterator. If the list is null, all available attribute
	 * information should be made accessible.
	 * 
	 * @param attributes
	 *            a list of attributes that the client is interested in
	 * @return an iterator providing access to the entire text and its selected
	 *         attributes
	 */
	 ...
	 * @exception IllegalArgumentException
	 *                if beginIndex is less then 0, endIndex is greater than the
	 *                length of the string, or beginIndex is greater than
	 *                endIndex.
	 */
	public AttributedCharacterIterator getIterator(Attribute[] attributes,
			int beginIndex, int endIndex) {
	...
	private boolean attributeValuesMatch(Set attributes, int runIndex1,
			int runIndex2) {
	...
	final private class AttributedStringIterator implements
		...
		public Object clone() {
			try {
				AttributedStringIterator other = (AttributedStringIterator) super
						.clone();
				return other;
...
class AttributeEntry implements Map.Entry {
	...
	public boolean equals(Object o) {
		...
		return other.key.equals(key)
				&& (value == null ? other.value == null : other.value
						.equals(value));
	}
